...
export default class Orders extends React.Component {
  ...
  getData = () => {
    fetch(Url.getClientProjects + cookie.get('userID'), { method: 'GET' })
      .then(res => res.json())
      .then(data => {
    ...
    if (this.state.loading || this.state.projects === [])
      ...
      return (
        <React.Fragment>
            ...
            <TableBody>
              {this.state.projects.map(project => (
                ...
                <TableRow key={project.Projid}>
                  ...
                  <Link to={{ pathname:'/client/floorplans/' , state:{ id: project.Projid }}}>
                  {project.Projname}
                  </Link>
                  </Grid>
                  </TableCell>
          ...
          <Link to={{ pathname:'/client/projects/'}} style={linkStyle}>
            See more projects
        </Link>
        </React.Fragment>
      );
